...
def select_model():
    ...
    c = input(
        f"\n{Fore.CYAN}Select [1-{len(MODEL_OPTIONS)}] or Enter for default: {Style.RESET_ALL}"
    ).strip()
    ...
    while time.time() < deadline:
        ...
        if len(s.turns) > last:
            for t in s.turns[last:]:
                ...
                print(
                    f"  {Fore.MAGENTA}LLM latency  : {t.llm_latency_ms:.0f} ms{Style.RESET_ALL}"
                )
                print(
                    f"  {Fore.MAGENTA}Total latency: {t.total_latency_ms:.0f} ms{Style.RESET_ALL}"
                )
                print(
                    f"  {Fore.MAGENTA}Tokens       : {t.total_tokens}{Style.RESET_ALL}"
                )

                if t.hallucination_score is not None:
                    c = Fore.RED if t.hallucination_score > 0.5 else Fore.GREEN
                    print(
                        f"  {c}Hallucination: {t.hallucination_score:.3f}{Style.RESET_ALL}"
                    )

                if t.faithfulness_score is not None:
                    print(
                        f"  {Fore.GREEN}Faithfulness : {t.faithfulness_score:.3f}{Style.RESET_ALL}"
                    )
        ...
        if s.status in ("completed", "failed", "busy", "no-answer"):
            print(
                f"\n{Fore.CYAN}CALL ENDED — Status: {s.status} | Turns: {s.total_turns}{Style.RESET_ALL}"
            )
            return
...
def main():
    ...
    confirm = input(
        f"\n{Fore.YELLOW}Call {to_number} now? [y/N]: {Style.RESET_ALL}"
    ).strip().lower()
